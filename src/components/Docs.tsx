...
export default function Docs({ isActive }: DocsProps) {
  ...
  const [groupByTags, setGroupByTags] = useState(() => {
    const stored = localStorage.getItem('docs-group-by-tags');
  ...
  const toggleGroupByTags = () => {
    const newValue = !groupByTags;
    setGroupByTags(newValue);
    localStorage.setItem('docs-group-by-tags', JSON.stringify(newValue));
  };
          ...
          <button
            ...
            className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 ${
              groupByTags
                ...
                : 'bg-dark-surface border border-dark-border text-dark-text-secondary hover:text-dark-text-primary'
            }`}
            title={groupByTags ? 'Show all documents' : 'Group by project tags'}
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
            </svg>
            {groupByTags ? 'Grouped' : 'Group'}
          </button>
        </div>
      </div>
                ...
                .map(doc => {
                  ...
                  return (
                    <button
                      ...
                      className="flex-shrink-0 w-24 flex flex-col items-center gap-2 p-3 bg-dark-surface border border-dark-border rounded-lg hover:border-dark-accent-primary transition-colors group"
                      title={doc.title}
                    >
                      {/* Doc type icon */}
        ...
        {filteredDocs.length === 0 ? (
          ...
          <div className="space-y-4">
            {Object.keys(groupedDocs).sort((a, b) => {
              ...
              const groupDocs = groupedDocs[groupName];
              const isCollapsed = collapsedGroups.has(groupName);
              const groupTag = availableTags.find(t => t.label === groupName);
              ...
              return (
                <div key={groupName} className="space-y-2">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-between p-3 bg-dark-surface border border-dark-border rounded-lg hover:border-dark-accent-primary transition-colors group"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-dark-text-muted">
                        {groupDocs.length} {groupDocs.length === 1 ? 'doc' : 'docs'}
                      </span>
                    </div>
                  </button>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start gap-2 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="text-sm font-medium text-dark-text-primary mb-1 whitespace-nowrap overflow-hidden group-hover:truncate">
                          {doc.title}
                        </h3>
                        {doc.isObsidian ? (
